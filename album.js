...
fetch(
  ...
  <div class="album-heder flex-coloumn px-2">
    ...
    <div class="d-flex">
      <img
        ...
        alt="small image"
      />
      <a class= "ancher-header text-white" href="../artist-page/index.html">${data.artist.name}</a>
      <div class="">
  ...
  timePlace.innerHTML = `${hours}hr ${minutes}min`
    ...
    data.tracks.data.map ((song, index) =>{
      ...
      <th class="TheaderText align-middle shortWidth p-1"scope="row">${index+1}</th>
      <td class="TheaderText align-middle p-1">${song.title} <br /><a class="link" href="#">${song.artist.name} </a></td>
      <td id="singleSongTime" class="TheaderText align-middle text-right p-1">${song.duration}</td>
    </tr>`
    })

    document.getElementById('botton-title').innerHTML= `<h2 class="text-white">More By
     ${data.artist.name}</h2>
    <h5 id="seeMore">SEE Discography</h5>`;
  });


  let songDurationPlace = document.getElementById('singleSongTime')
  let total = song.duration;
  let minute = Math.floor(total / 60);
  let second = total % 60;
  songDurationPlace.innerHTML = `${minute}:${second}`
